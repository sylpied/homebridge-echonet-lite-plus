...
撤去した機器は、検出機器カードの「この機器を削除」から検出履歴と保存済み機器設定を個別に削除できます。「検出履歴をクリア」は一覧全体を消去します。削除後は設定を保存してChild Bridgeを再起動してください。個別削除した機器が将来再びECHONET Lite応答を返した場合は、新しい検出として一覧へ戻ります。
