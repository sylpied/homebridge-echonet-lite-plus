...
Homebridge 2のMatter機能を有効にした場合、意味がMatter標準デバイスタイプと明確に一致する機器をMatterにも公開します。

| Matterでの扱い | ECHONET Lite機器（EOJ） |
| --- | --- |
| Thermostat / Room Air Conditioner | 家庭用エアコン `0130`、床暖房 `027B` |
| Fan | 換気扇 `0133`、空調換気扇 `0134`、空気清浄機 `0135`、浴室暖房乾燥機 `0273` |
| Window Covering | 電動雨戸・シャッター `0263` |
| Door Lock | 電気錠 `026F` |
| Dimmable Light | 一般照明 `0290`、単機能照明 `0291` |
| On/Off Switch | 電気温水器 `026B`、瞬間式給湯器 `0272`、ハイブリッド給湯機 `02A6`、JEM-A/HA対応スイッチ `05FD` |
| Sensor | 温度センサー `0011`、湿度センサー `0012` |

給湯器は機器全体の電源ではなく、ECHONET Liteで意味が確定している「風呂自動」または「自動沸き上げ」のON/OFFとして公開します。浴室暖房乾燥機のMatter Fanは運転／停止を扱い、換気・暖房・乾燥などの詳細モードはHomeKit側の個別Switchで扱います。

電力、太陽光発電、EV充放電、ガス、水道などは、現在使用できるMatter標準デバイスタイプへ意味を変えずに割り当てられないためMatterには公開しません。電動窓 `0265`も同梱MRA 1.4.0に機器定義がないため、現時点では対象外です。
...
撤去した機器は、検出機器カードの「この機器を削除」から検出履歴と保存済み機器設定を個別に削除できます。カードは削除操作の直後に一覧から消え、設定保存とChild Bridge再起動後も自動探索では復活しません。「検出履歴をクリア」は一覧全体と個別削除の記録を消去するため、現在応答している機器を改めて検出したい場合にも利用できます。
